...
{
    public partial class common_cons : Form
    {
        ...
        private async void User_Load(object sender, EventArgs e)
        {
            ...
            while (await sqlReader1.ReadAsync())
            {
                ...
                {
                    if (Convert.ToString(sqlReader1["Статус"]) == "Консультант")
                    {
        ...
        private async void button3_Click(object sender, EventArgs e)
        {
            ...
            OleDbCommand command = new OleDbCommand("UPDATE [Клиенты] SET [Статус]=@status WHERE [Код]=@pri", SqlConnection);
            command.Parameters.AddWithValue("status", "Главный Консультант");
            command.Parameters.AddWithValue("pri", textBox9.Text);
            await command.ExecuteNonQueryAsync();
            await command.ExecuteNonQueryAsync();
